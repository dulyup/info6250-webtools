...
function thinkAbout( wordInfo ) {
  ...
  for (let i = 0; i < wordInfo.allWords.length; i++) {
    let eachWord = wordInfo.allWords[i];
    let index = eachWord[0];
    if (!outputs[index]) {
      outputs[index] = [];
    }
    outputs[index].push(eachWord);
  }
  return outputs;
  // EDIT ABOVE THIS
}
...
function pickGuess( wordInfo, history ) {
  ...
  // EDIT BELOW THIS
  let index = wordInfo.word[0];
  let arr = wordInfo.magic[index];
  return arr[history.count - 1];
  // EDIT ABOVE THIS
}
